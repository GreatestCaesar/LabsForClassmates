...
public class Ship implements Liner, Sailboat{
    ...
    public Ship(int quantity, double price, int day, int payback, double cost) {
    ...
    public Object[] Input() {
        ...
        }catch (Exception e)       																				// ИСКЛЮЧЕНИЕ С ЗАВЕРШЕНИЕМ РАБОТЫ
        {
            System.out.println("Ошибка! опять введена ерунда\n" + e.toString());
            System.exit(0);
        }
        System.out.println();
        return input;
