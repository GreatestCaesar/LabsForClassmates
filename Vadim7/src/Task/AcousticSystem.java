...
public class AcousticSystem { // акустическая система
    ...
    public void vol(int n){ // перегруженный мметод
        if(n>volume){

        }else {
            System.out.println("Звук уменьшен на " + n);
            volume -= n;
        }
    }
    ...
    public void tools(String ... s){ // перегруженный с изменяемым числом входныъ данных
        System.out.println("Музыкальные инструменты, совместимые с системой: ");
        for(String str:s){
    ...
    public String blue(String s){
        ...
        switch (s) {
            case "on": {
                str = "Bluetooth включен";
                break;
            }
            case "off":{
                str = "Bluetooth выключен";
                break;
            }
        }
        return str;
